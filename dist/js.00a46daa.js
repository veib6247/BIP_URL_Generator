...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"node_modules/vue/dist/vue.esm.browser.js":[function(require,module,exports) {
...
function invokeWithErrorHandling(handler, context, args, vm, info) {
  ...
  try {
    ...
    if (res && !res._isVue && isPromise(res) && !res._handled) {
      res.catch(e => handleError(e, vm, info + ` (Promise/async)`)); // issue #9511
...
const comment = /^<!\--/;
const conditionalComment = /^<!\[/; // Special Elements (can contain anything)
...
const emptySlotScopeToken = `_empty_`; // configurable state
...
function parse(template, options) {
  ...
  parseHTML(template, {
    ...
    start(tag, attrs, unary, start, end) {
      ...
      const element = stack[stack.length - 1]; // pop stack
...

  {
    if (el.tag === 'template') {
      ...
      if (slotBinding) {
        ...
        slotContainer.slotScope = slotBinding.value || emptySlotScopeToken; // remove children as they are returned from scopedSlots now
...
const fnInvokeRE = /\([^)]*?\);*$/;
const simplePathRE = /^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*|\['[^']*?']|\["[^"]*?"]|\[\d+]|\[[A-Za-z_$][\w$]*])*$/; // KeyboardEvent.keyCode aliases
...
function generate(ast, options) {
  const state = new CodegenState(options); // fix #11483, Root level <script> tags should not be rendered.
...
function genScopedSlots(el, slots, state) {
  ...
  let needsKey = !!el.if; // OR when it is inside another scoped slot or v-for (the reactivity may be
...
function genScopedSlot(el, state) {
  ...
  const fn = `function(${slotScope}){` + `return ${el.tag === 'template' ? el.if && isLegacySyntax ? `(${el.if})?${genChildren(el, state) || 'undefined'}:undefined` : genChildren(el, state) || 'undefined' : genElement(el, state)}}`; // reverse proxy v-slot without scope on this.$slots
...
var _vueEsm = _interopRequireDefault(require("../node_modules/vue/dist/vue.esm.browser"));
...
new _vueEsm.default({
  ...
  methods: {
    ...
    generateURL: function generateURL() {
      var bipURL = "https://"; // check for mode, if "TEST", append subdomain
      ...
      bipURL += "".concat(this.domain.input, "/bip/login?");

      switch (this.selectedEntityType) {
        case "DIVISION":
          bipURL += "division_name=".concat(this.entityName.input);
          break;

        case "MERCHANT":
          bipURL += "merchant_name=".concat(this.entityName.input);
          break;

        case "CHANNEL":
          bipURL += "channel_name=".concat(this.entityName.input);
          break;
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "9122" + '/');
